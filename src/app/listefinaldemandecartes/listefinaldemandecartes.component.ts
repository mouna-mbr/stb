...
@Component({
  selector: 'app-listefinaldemandecartes',
  standalone: true,
  imports: [CommonModule, NavbarComponent, PdfGeneratorComponent, ReactiveFormsModule], 
   templateUrl: './listefinaldemandecartes.component.html',
  styleUrl: './listefinaldemandecartes.component.css'
})
export class ListefinaldemandecartesComponent  implements OnInit {
      ...
      (res) => {
        this.listDemandeCartes = res.filter((demande: Demandecartes) => {
          ...
          return (
            demande.validationI === true &&
            demande.validationF === false &&
            demande.statut.trim() === "Acceptée Dans La Validation Initiale" &&
    ...
    this.demandeService.getAllDemandeCarte().subscribe(
      (res) => {
        this.listDemandeCartes = res.filter((demande: Demandecartes) => {
          const demandeDate = new Date(demande.time);
          demandeDate.setHours(0, 0, 0, 0); // Remettre à zéro les heures, minutes, secondes et millisecondes pour comparer la date uniquement
          return  demande.validationI === true &&
          demande.validationF === false &&
          demande.statut.trim() === "Acceptée Dans La Validation Initiale" &&
          demandeDate.getTime() === yesterday.getTime();
        });
      ...
      (res) => {
        this.listDemandeCartes = res.filter((demande: Demandecartes) => {
          ...
          return demande.validationI === true &&
                  demande.validationF === false &&
                  demande.statut.trim() === "Acceptée Dans La Validation Initiale" &&
                  demandeDate >= startOfWeek && demandeDate <= today;
        });
